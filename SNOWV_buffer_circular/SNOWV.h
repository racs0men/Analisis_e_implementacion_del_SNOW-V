...
struct SnowV32 {
    u16 A[16], B[16];
    u32 R1[4], R2[4], R3[4];
    int Pi_A,Pf_A,coefA[3];
    int coeficientes_LENGTH = 3;
    int Pi_B,Pf_B,coefB[3];
    int cont;
  ...
  void fsm_update(void) {
    ...
    for (int i = 0; i < 4; i++) {
      u32 T2 = MAKEU32(A[(Pi_A + (2 * i + 1)) % 16], A[(Pi_A + (2 * i)) % 16]);
      R1[i] = (T2 ^ R3[i]) + R2[i];
    }
  ...
  void lfsr_update(void) {
    ...
    for (int i = 0; i < 8; i++) {
        u16 u = mul_x(A[coefA[0]], 0x990f) ^ A[coefA[1]] ^ mul_x_inv(A[coefA[2]], 0xcc87) ^ B[coefB[0]];
        u16 v = mul_x(B[coefB[0]], 0xc963) ^ B[coefB[1]] ^ mul_x_inv(B[coefB[2]], 0xe4b1) ^ A[coefA[0]];
  ...
  void keystream(u8 * z) {
    ...
    for (int i = 0; i < 4; i++) {
      u32 T1 = MAKEU32(B[(Pi_B + (2 * i + 9)) % 16], B[(Pi_B + (2 * i + 8)) % 16]);
      u32 v = (T1 + R1[i]) ^ R2[i];
      z[i * 4 + 0] = (v >> 0) & 0xff;
      z[i * 4 + 1] = (v >> 8) & 0xff;
      z[i * 4 + 2] = (v >> 16) & 0xff;
  ...
  void keyiv_setup(u8 * key, u8 * iv, int is_aead_mode) {
    ...
    for (int i = 0; i < 8; i++) {
      A[(Pi_A + i) % 16] = MAKEU16(iv[2 * i + 1], iv[2 * i]);
      A[(Pi_A + (i + 8) % 16)] = MAKEU16(key[2 * i + 1], key[2 * i]);
      B[(Pi_B + i) % 16] = 0x0000;
      B[(Pi_B + (i + 8) ) % 16] = MAKEU16(key[2 * i + 17], key[2 * i + 16]);
    }
    ...
    for (int i = 0; i < 16; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        A[(Pi_A + (j + 8)) % 16] ^= MAKEU16(z[2 * j + 1], z[2 * j]);
      }
      if (i == 14) {
        for (int j = 0; j < 4; j++) {

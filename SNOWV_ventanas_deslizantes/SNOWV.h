...
struct SnowV32 {
  ...
  void fsm_update(void) {
    ...
    for (int i = 0; i < 4; i++) { 
      u32 T2 = MAKEU32(A[(punteroK - (LFSR_LENGHT + 1) + (2 * i + 1)) % 16 ], A[(punteroK - (LFSR_LENGHT + 1) + (2 * i)) % 16]);
      R1[i] = (T2 ^ R3[i]) + R2[i];
    }
  ...
  void lfsr_update(void) {
    ...
    for (int i = 0; i < 8; i++) {
      u32 u = mul_x(A[punteroK - (LFSR_LENGHT + 1)], 0x990f) ^ A[(punteroK - (LFSR_LENGHT + 1)) + 1] ^
              mul_x_inv(A[(punteroK - (LFSR_LENGHT + 1)) + 8], 0xcc87) ^ B[punteroKB - (LFSR_LENGHT + 1)];
      u32 v = mul_x(B[punteroKB - (LFSR_LENGHT + 1)], 0xc963) ^ B[(punteroKB - (LFSR_LENGHT + 1)) + 3] ^
              mul_x_inv(B[(punteroKB - (LFSR_LENGHT + 1)) + 8], 0xe4b1) ^ A[punteroK - (LFSR_LENGHT + 1)];

      A[punteroK - (LFSR_LENGHT + 1)] = A[punteroK] = u;
      B[punteroKB - (LFSR_LENGHT + 1)] = B[punteroKB] = v;

      if((++punteroK) == (LFSR_LENGHT2 + 2))
        punteroK = (LFSR_LENGHT + 1);

      if((++punteroKB) == (LFSR_LENGHT2 + 2))
        punteroKB = (LFSR_LENGHT + 1);
    }
  ...
  void keystream(u8 * z) {
    ...
    for (int i = 0; i < 4; i++) { 
      u32 T1 = MAKEU32(B[(punteroKB - (LFSR_LENGHT + 1)) + (2 * i + 9)], B[(punteroKB - (LFSR_LENGHT + 1)) + (2 * i + 8)]);
      u32 v = (T1 + R1[i]) ^ R2[i];
      z[i * 4 + 0] = (v >> 0) & 0xff;
      z[i * 4 + 1] = (v >> 8) & 0xff;
      z[i * 4 + 2] = (v >> 16) & 0xff;
  ...
  void printLFSRA(u16 * A, int puntero) {
    cout << "*****Mostrar LFSR********\n";
      for (int i = 0; i < 16; i++) {
        printf ("%x|", A[(punteroK + i) % 32]); 
      }
    cout << "\n"; 
  }
  ...
  void keyiv_setup(u8 * key, u8 * iv, int is_aead_mode) {
    ...
    for (int i = 0; i < 8; i++) { 
      A[punteroK - (LFSR_LENGHT + 1) + i] = MAKEU16(iv[2 * i + 1], iv[2 * i]);
      A[(punteroK + i) % 32] = MAKEU16(iv[2 * i + 1], iv[2 * i]);
      A[punteroK - (LFSR_LENGHT + 1) + (i + 8)] = MAKEU16(key[2 * i + 1], key[2 * i]);
      A[(punteroK + (i + 8)) % 32] = MAKEU16(key[2 * i + 1], key[2 * i]);
      B[punteroKB - (LFSR_LENGHT + 1) + i] = 0x0000;
      B[(punteroKB + i) % 32] = 0x0000;
      B[punteroKB - (LFSR_LENGHT + 1) + (i + 8)] = MAKEU16(key[2 * i + 17], key[2 * i + 16]);
      B[(punteroKB + (i + 8)) % 32] = MAKEU16(key[2 * i + 17], key[2 * i + 16]);
    }

    if(is_aead_mode == 1) {
      B[punteroKB - (LFSR_LENGHT + 1) + 0] = 0x6C41;
      B[punteroKB - (LFSR_LENGHT + 1) + 1] = 0x7865;
      B[punteroKB - (LFSR_LENGHT + 1) + 2] = 0x6B45;
      B[punteroKB - (LFSR_LENGHT + 1) + 3] = 0x2064;
      B[punteroKB - (LFSR_LENGHT + 1) + 4] = 0x694A;
      B[punteroKB - (LFSR_LENGHT + 1) + 5] = 0x676E;
      B[punteroKB - (LFSR_LENGHT + 1) + 6] = 0x6854;
      B[punteroKB - (LFSR_LENGHT + 1) + 7] = 0x6D6F;
    }
    ...
    for (int i = 0; i < 16; i++) { 
      ...
      for (int j = 0; j < 8; j++) {
        A[punteroK - (LFSR_LENGHT + 1) + (j + 8)] ^= MAKEU16(z[2 * j + 1], z[2 * j]);
        A[(punteroK + (j + 8)) % 32] ^= MAKEU16(z[2 * j + 1], z[2 * j]);
      }
      if (i == 14) {
        for (int j = 0; j < 4; j++) {
